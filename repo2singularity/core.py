...
import docker
from repo2docker.utils import is_valid_docker_image_name, validate_and_generate_port_mapping

from . import Repo2Singularity, __version__
...
def get_argparser():
    ...
    argparser.add_argument(
        'repo', help=('Path/URL to a git repository that should be built.'),
    )
    ...
    argparser.add_argument(
        '--ref', help=('If building a git url, which reference to check out. E.g., `master`.'),
    )
    ...
    argparser.add_argument(
        '--build-memory-limit', help='Total Memory that can be used by the docker build process',
    )

    argparser.add_argument(
        'cmd', nargs=argparse.REMAINDER, help='Custom command to run after building container',
    )
    ...
    argparser.add_argument(
        '--push', dest='push', action='store_true', help='Push docker image to repository',
    )
    ...
    argparser.add_argument('--user-id', help='User ID of the primary user in the image', type=int)

    argparser.add_argument('--user-name', help='Username of the primary user in the image')
    ...
    argparser.add_argument('--target-repo-dir', help=Repo2Singularity.target_repo_dir.help)
    ...
    argparser.add_argument(
        '--cache-from', action='append', default=[], help=Repo2Singularity.cache_from.help,
    )
...
def make_r2s(argv=None):
    ...
    if args.editable:
        ...
        if os.path.isdir(args.repo):
            ...
            r2s.log.error(
                'Cannot mount "{}" in editable mode ' 'as it is not a directory'.format(args.repo),
